...
public interface OwnerRepository extends JpaRepository<Owner, Integer> {
   ...
   *
   * <p>This method returns an {@link Optional} containing the {@link Owner} if found. If no {@link
   * Owner} is found with the provided id, it will return an empty {@link Optional}.
   *
   * @param id the id to search for
   * @return an {@link Optional} containing the {@link Owner} if found, or an empty {@link Optional}
   *     if not found.
   * @throws IllegalArgumentException if the id is null (assuming null is not a valid input for id)
   */
