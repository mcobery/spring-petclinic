...
/**
 * This test ensures that there are no hard-coded strings without internationalization in any HTML
 * files. Also ensures that a string is translated in every language to avoid partial translations.
 *
...
public class I18nPropertiesSyncTest {
  ...
  private static final Pattern BRACKET_ONLY =
      Pattern.compile("<[^>]*>\\s*[\\[\\]](?:&nbsp;)?\\s*</[^>]*>");

  private static final Pattern HAS_TH_TEXT_ATTRIBUTE =
      Pattern.compile("th:(u)?text\\s*=\\s*\"[^\"]+\"");
    ...
    for (Path file : files) {
      ...
      for (int i = 0; i < lines.size(); i++) {
        ...
        if (line.startsWith("//")
            || line.startsWith("@")
            || line.contains("log.")
            || line.contains("System.out")) continue;
  ...
  public void checkI18nPropertyFilesAreInSync() throws IOException {
    ...
    try (Stream<Path> stream = Files.walk(Paths.get(I18N_DIR))) {
      propertyFiles =
          stream
              .filter(p -> p.getFileName().toString().startsWith(BASE_NAME))
